...
def update_incremental_report(scan_type, scan_path):
    """Update the existing incremental report with new findings."""
    try:
        # Check if we already have an incremental report for this scan session
        existing_reports = [f for f in os.listdir(REPORT_OUTPUT_DIR) 
                          if f.startswith(f"pii_scan_report_{scan_type}_incremental_") 
                          and f.endswith(".html")]
        
        if existing_reports:
            # Use the most recent incremental report
            existing_reports.sort()
            report_filename = existing_reports[-1]
            report_path = os.path.join(REPORT_OUTPUT_DIR, report_filename)
            logger.info(f"Updating existing incremental report: {report_filename}")
        else:
            # Create new incremental report
            timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
            report_filename = f"pii_scan_report_{scan_type}_incremental_{timestamp}.html"
            report_path = os.path.join(REPORT_OUTPUT_DIR, report_filename)
            logger.info(f"Creating new incremental report: {report_filename}")
        
        # Generate updated HTML content with all current results
        html_content = generate_html_content(scan_type, scan_path, incremental=True)
        
        # Write updated HTML report to file
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        logger.info(f"Incremental report updated: {report_path}")
        return report_path
        
    except Exception as e:
        logger.error(f"Error updating incremental report: {e}")
        return None

def generate_html_content(scan_type, scan_path, incremental=False):
    """Generate HTML content (separated from file writing for reuse)."""
    ...
    # Use stored file sizes instead of trying to access files that may not exist
    total_size_bytes = sum(result["file_size"] for result in scan_results)
...
"""
    ...
    # Add incremental indicator
    incremental_note = ""
    if incremental:
        incremental_note = f"""
            <div class="scan-info" style="background: #fff3cd; border-left-color: #ffc107;">
                <h3>⚠️ Incremental Report (Live Updates)</h3>
                <p><strong>Note:</strong> This report is updated in real-time as new PII is detected.</p>
                <p><strong>Last Updated:</strong> {datetime.now().strftime('%Y-%m-%d at %H:%M:%S')}</p>
                <p><strong>Files Scanned So Far:</strong> {total_files}</p>
                <p><strong>PII Detected In:</strong> {files_with_pii} files</p>
                <p><strong>Total PII Entities:</strong> {total_pii_entities}</p>
            </div>
        """
    ...
    <head>
        ...
        <title>PII Scanner Report - {scan_type.title()} Scan (Live Updates)</title>
        <style>
            body {{
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>PII Scanner Report (Live Updates)</h1>
                <p>Generated on {datetime.now().strftime('%Y-%m-%d at %H:%M:%S')}</p>
                <p>Scan Type: {scan_type.title()} | Scan Path: {scan_path}</p>
                <p>Report Type: Incremental with Real-time Updates</p>
            </div>

            {incremental_note}

            <div class="summary">
                <h2>Scan Summary (Live)</h2>
                <div class="stats">
                    <div class="stat">
            ...
            <div class="footer">
                ...
                <p><strong>Report Type:</strong> Incremental with Real-time Updates</p>
...
def generate_html_report(scan_type, scan_path):
    ...
    try:
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d_%H-%M-%S")
        report_filename = f"pii_scan_report_{scan_type}_final_{timestamp}.html"
        report_path = os.path.join(REPORT_OUTPUT_DIR, report_filename)
        
        # Generate HTML content
        html_content = generate_html_content(scan_type, scan_path, incremental=False)
        
        # Write HTML file
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        logger.info(f"Final HTML report generated successfully: {report_path}")
...
def scan_file_for_pii(file_path, scan_type):
    ...
    try:
        ...
        if all_pii_entities:
            ...
            # IMMEDIATE OUTPUT for Veeam detection
            print("PII data potentially exposed")  # For Veeam regex match
            print(f"PII_DETECTED: {labels}")  # For structured output
            
            # IMMEDIATE incremental report update (same report, updated content)
            logger.info("PII detected - updating incremental report immediately...")
            try:
                # Store result first so it's available for the report
                file_size = os.path.getsize(file_path)
                file_modified = datetime.fromtimestamp(os.path.getmtime(file_path), tz=timezone.utc).isoformat()
                store_scan_result(file_path, all_pii_entities, file_size, file_modified, scan_type)
                
                # Update the SAME incremental report with new findings
                report_path = update_incremental_report(scan_type, "Current Scan")
                logger.info(f"Incremental report updated with new PII: {report_path}")
                print(f"INCREMENTAL_REPORT_UPDATED: {report_path}")  # For Veeam to capture updated report path
            except Exception as e:
                logger.error(f"Error updating incremental report: {e}")
...
if __name__ == "__main__":
    try:
        ...
        for root, _, files in os.walk(args.path):
            for file in files:
                ...
                try:
                    ...
                    # Check if PII was found in this file
                    for result in scan_results:
                        if result["file_path"] == file_path and result["has_pii"]:
                            pii_found = True
                            # For lite scans, exit immediately after first PII detection
                            if args.scan_type == "lite":
                                logger.info("PII detected in lite scan - exiting for Veeam to mark recovery point as risky")
                                # Generate final report before exiting
                                try:
                                    report_path = generate_html_report(args.scan_type, args.path)
                                    logger.info(f"Final HTML report generated before exit: {report_path}")
                                    print(f"FINAL_REPORT: {report_path}")  # For Veeam to capture final report path
                                except Exception as e:
                                    logger.error(f"Error generating final report: {e}")
                                
                                # Exit with PII found code so Veeam marks recovery point as risky
                                sys.exit(EXIT_PII_FOUND)
                            break
        ...
        if scan_errors:
            ...
            print(f"\nWARNING: {len(scan_errors)} errors occurred during scanning. Check logs for details.")

        # Generate final HTML report (for full scans or when no PII found)
        try:
        ...
        logger.info(f"Scanning complete. Processed {files_processed} files.")
